...
def main():
    ...
    def opponent_is_in_check(piece_id, initial_yx, selected_yx) -> (list, list):
        ...
        if np.all(king_closest_obstructing_pieces_is[turn_i_current_opponent[1]] == initial_yx, axis=1).any():
            ...
            obstruction_check_yxs = return_potential_moves(initial_yx, (0,), vector_magnitude_overwrite=((np.array([vector_sign]),), (9,)))
    ...
    def opponent_has_no_valid_moves(piece_id, initial_yx, selected_yx) -> bool:
    ...
    def draw_selected_move(piece_next_yx, output_img_loc: Img):
        ...
        if in_drawn_idxs.shape[0] != 0:
            ...
            if piece_id != Pieces.king.value:
                ...
                for obstruction_array, king_yx in zip(king_closest_obstructing_pieces_is, king_positions_yx):
                    a = obstruction_array == piece_initial_yx
                    t = np.all(a, axis = 1)
                    ...
                    if np.any(selected_piece_vector == 0) or np.all(np.abs(selected_piece_vector) % 2 == 0):
                        ...
                        obstruction_array[obstruction_i] = return_potential_moves(king_yx, (0,), check_type='obstruction', vector_magnitude_overwrite=((np.array([selected_piece_sign]),), (9,)))
            ...
            if opponent_has_no_valid_moves(piece_id, piece_initial_yx, piece_next_yx):
                if king_in_check_valid_piece_idxs[turn_i_current_opponent[0]] is None:
                    print('checkmate')
                else:
                    print('stalemate')
